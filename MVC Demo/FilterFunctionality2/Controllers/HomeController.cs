...
namespace FilterFunctionality.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index(string Search, string searchBy, int? page)
        {
            if (searchBy == "Name")
            {
                return View(db.Employee1.Where(x => x.Name.StartsWith(Search) || Search == null).ToList().ToPagedList(page ?? 1 , 3));
            }
            else
            {
                return View(db.Employee1.Where(x => x.Gender.Equals(Search) || Search == null).ToList().ToPagedList(page ?? 1, 3));
            }
           
        }

        public ActionResult Index1(string Search, string searchBy, int page)
        {
            if (searchBy == "Name")
            {
